...
namespace OVAWORKT
{
    ...
    {
        public static SerializedProperty GetArrayFromElement(this SerializedProperty element)
        {
            StringBuilder propertyPath = new();
            string[] parts = element.propertyPath.Split('.');
            for (int i = 0; i < parts.Length; i++)
            {
                if (parts[i] == "Array") break;
                else if (i > 0) propertyPath.Append('.');
                propertyPath.Append(parts[i]);
            }

            return element.serializedObject.FindProperty(propertyPath.ToString());
        }
        ...
        public static SerializedProperty FindPropertyRelativeOrFail(this SerializedProperty property, string path)
        {
            ...
            if (subProperty == null) throw new InvalidOperationException($"Failed to find property '{SerializedObjectLabel(property.serializedObject)}.{property.propertyPath}.{path}'.");
            return subProperty;
        }
        ...
        public static SerializedProperty FindPropertyOrFail(this SerializedObject serializedObject, string path)
        {
            ...
            if (property == null) throw new InvalidOperationException($"Failed to find property '{SerializedObjectLabel(serializedObject)}.{path}'.");
            return property;
        }
        ...
        public static string[] PropertyPathParts(this SerializedProperty property) => property.FixedPropertyPath().Split('.');
        ...
        private static void EnsureReflectable(SerializedProperty property, bool allowMultiple = false)
        {
            ...
            if (!allowMultiple && property.serializedObject.isEditingMultipleObjects) throw new NotSupportedException($"Attempting to reflect property '{property.propertyPath}' on multiple objects.");
            if (property.serializedObject.targetObject == null) throw new NotSupportedException($"Attempting to reflect property '{property.propertyPath}' on a null object.");
        }

        private static string SerializedObjectLabel(SerializedObject serializedObject) => serializedObject.isEditingMultipleObjects ? "[Multiple]" : serializedObject.targetObject.GetType().Name;
        ...
        private static void GetUnderlying(this SerializedProperty property, out object parent, out FieldInfo field, out object value)
        {
            ...
            {
                var part = parts[i];
                if (parent == null) throw new NullReferenceException($"Parent of '{SerializedObjectLabel(property.serializedObject)}.{string.Join(".", parts, 0, i + 1)}' is null.");

                field = GetPropertyPartField(part, parent);
                if (i < parts.Length - 1) parent = GetPropertyPartValue(part, parent);
                else value = GetPropertyPartValue(part, parent);
            }
        }
        ...
        private static void SetUnderlying(this SerializedProperty property, object value)
        {
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                if (i < parts.Length - 1) parent = GetPropertyPartValue(part, parent);
                else SetPropertyPartValue(part, parent, value);
            }
        }
        ...
        private static object GetPropertyPartValue(string propertyPathPart, object parent)
        {
            ...
            if (IsPropertyIndexer(propertyPathPart, out var fieldName, out var index)) return ((IList)value)[index];
            else return value;
        }
        ...
        private static void SetPropertyPartValue(string propertyPathPart, object parent, object value)
        {
            ...
            if (IsPropertyIndexer(propertyPathPart, out var fieldName, out var index)) ((IList)field.GetValue(parent))[index] = value;
            else field.SetValue(parent, value);
        }
        ...
        public static T ReadEnum<T>(this SerializedProperty property) where T : Enum
        {
            ...
            if (property.propertyType == SerializedPropertyType.Enum) return (T)Enum.ToObject(typeof(T), property.enumValueIndex);
            else if (property.propertyType == SerializedPropertyType.Integer) return (T)Enum.ToObject(typeof(T), property.intValue);
        ...
        public static void WriteEnum<T>(this SerializedProperty property, T value) where T : Enum
        {
            ...
            // When using SerializeReference instead of SerializeField on a parent object,
            // the underlying property type becomes integer instead of enum.
            if (property.propertyType == SerializedPropertyType.Enum) property.enumValueIndex = Convert.ToInt32(value);
            else if (property.propertyType == SerializedPropertyType.Integer) property.intValue = Convert.ToInt32(value);
            else throw new NotSupportedException();
        }

        public static string[] ReadStringArray(this SerializedProperty property) => property.ReadArray(p => p.stringValue);
        public static void WriteStringArray(this SerializedProperty property, string[] array) => property.WriteArray(array, (p, v) => p.stringValue = v);
        public static int[] ReadIntArray(this SerializedProperty property) => property.ReadArray(p => p.intValue);
        public static void WriteIntArray(this SerializedProperty property, int[] array) => property.WriteArray(array, (p, v) => p.intValue = v);
        public static byte[] ReadByteArray(this SerializedProperty property) => property.ReadArray(p => (byte)p.intValue);
        public static void WriteByteArray(this SerializedProperty property, byte[] array) => property.WriteArray(array, (p, v) => p.intValue = v);
        ...
        public static T[] ReadArray<T>(this SerializedProperty property, Func<SerializedProperty, T> getValue)
        {
            ...
            for (var i = 0; i < array.Length; i++) array[i] = getValue(property.GetArrayElementAtIndex(i));
            return array;
        }
        ...
        public static void WriteArray<T>(this SerializedProperty property, T[] array, Action<SerializedProperty, T> setValue)
        {
            ...
            for (var i = 0; i < array.Length; i++) setValue(property.GetArrayElementAtIndex(i), array[i]);
        }
        #endregion
    }
}